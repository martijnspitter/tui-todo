...
func EmptyNothingFoundView(translator *i18n.TranslationService, width, height int) string {
	...
	emptySubTitle := styling.SubtextStyle.Bold(true).Width(width).Align(lipgloss.Center).Render(translator.T("feedback.try_different_search"))
	cactusText := styling.EmptyStyle.PaddingRight(2).Render(cactus)
	emptyState := lipgloss.JoinVertical(lipgloss.Center, emptyTitle, emptySubTitle, cactusText)
